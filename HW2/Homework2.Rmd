...
set.seed(1)
y.post.sim1an_nba <- rpois(sim.points, lambda = post.an.nba) # First using the analytical posterior (conjugate)

set.seed(1)
post.samples.nba <- sample(lambda, size = sim.points, replace = T, prob = posterior.nba)

set.seed(1)
y.post.sim1sim_nba <- rpois(sim.points, lambda = post.samples.nba) # Secondly using the simulated posterior from earlier.

set.seed(1)
post.an.acb <- rgamma(sim.points, shape = posterior.par.acb[1], rate = posterior.par.acb[2])

set.seed(1)
y.post.sim1an_acb <- rpois(sim.points, lambda = post.an.acb) # First using the analytical posterior (conjugate)

set.seed(1)
post.samples.acb <- sample(lambda, size = sim.points, replace = T, prob = posterior.acb)

set.seed(1)
y.post.sim1sim_acb <- rpois(sim.points, lambda = post.samples.acb) # Secondly using the simulated posterior from earlier.
...
Both the "fully simulated" and "half-simulated" posterior predictive distributions look very similar. 
